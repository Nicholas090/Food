...
function calc() {
    ...
    function calcTotal() {
        if (!sex || !height || !weight || !age || !ratio) {
            result.textContent = 'Выбери все параметры,Сука';
            return;
        }
        if (sex === 'female') {
            Math.round(result.textContent = (447.6 + (9.2 * weight) + (3.1 * height) - (4.3 * age)) * ratio);
        } else {
            Math.round(result.textContent = (88.36 + (13.4 * weight) + (4.8 * height) - (5.7 * age)) * ratio);
        }
    }
    calcTotal();
...
window.addEventListener('DOMContentLoaded', () => {
    const modalTimerId = setTimeout(()=> (0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.openModal)('.modal',modalTimerId ), 322000);

(0,_modules_tabs__WEBPACK_IMPORTED_MODULE_0__.default)('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active');
(0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.default)('[data-modal]', '.modal', modalTimerId);
(0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__.default)('.timer', '2021-06-12');
